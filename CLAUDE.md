...
- `scripts/15_export_model_params.py` — recomputes every parameter the live calculator needs fresh from source and writes `site/model_params.json`; never hand-edit that JSON
- `site/index.html` — the public calculator (vanilla HTML/CSS/JS, no framework, no build step); fetches `model_params.json` and does all math client-side
...
- `.github/workflows/pages.yml` — deploys `site/` to GitHub Pages on push to main (path-filtered to `site/**`)
...
- Export calculator params (15, no MLPerf clone needed): `python3 scripts/15_export_model_params.py` -- re-run after any upstream model/data change, then `pytest tests/test_reproduction.py -k model_params` to confirm
...
- Live calculator (15, DONE): public static site at
  https://elyryba.github.io/gpu-topology-efficiency/ (GitHub Pages, deployed by
  .github/workflows/pages.yml on push to main). Zero server-side logic -- fetches
  site/model_params.json and computes the discount multiplier/band/interpolation flag
  client-side using 09's exact formulas. model_params.json is exported fresh from source
  by 15_export_model_params.py (never hand-edited) and locked by a test
  (test_model_params_json_matches_committed_copy) that regenerates it in-memory and
  asserts an exact match modulo the generated_at timestamp -- the site cannot silently
  drift from the fitted models. Caught one real bug while building 15: the gen speed
  multipliers must be fit from 08's spec (no comm interaction term), NOT reused from 09's
  fit (which has the domain_mined_x_comm interaction) -- the two specs give measurably
  different gen coefficients since the interaction term soaks up some of what would
  otherwise load onto the gen dummies.
...
- Never hand-edit site/model_params.json — it's generated by 15_export_model_params.py and
  locked by a test against silent drift; if a model/data change should change the site,
  re-run 15 and let the test confirm it, don't patch the JSON directly
- When adding a gen-FE-derived quantity, always check which regression SPEC you're pulling
  coefficients from (08's vs. 09's differ because of 09's comm interaction term) — reusing
  the wrong fit's coefficients is a real, easy-to-make mistake (see the live-calculator bug
  above), not just a hypothetical one
